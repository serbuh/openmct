...
export default function () {
  return function install(openmct) {
    ...
    openmct.types.addType('udp_video.imagery', {
      key: 'udp_video.imagery',
      name: 'UDP VIdeo Imagery',
      cssClass: 'icon-image',
      description:
...
function getRealtimeProvider(openmct) {
  // console.log("Imagery getRealtimeProvider")
  return {
    supportsSubscribe: (domainObject) => domainObject.type === 'udp_video.imagery',
    subscribe: (domainObject, callback) => {
      // const video_key = domainObject.name // TODO: Update to set a different key to each video source 
      window.socket.emit("subscribe-video", domainObject.configuration.imagePortSource)
      window.socket.on("video-point", msg => {
        callback(msg);
      })
      return () => {
        window.socket.emit("unsubscribe-video", domainObject.configuration.imagePortSource)
      };
    }
  };
}
...
function getHistoricalProvider(openmct) {
  return {
    supportsRequest: (domainObject, options) => {
      return domainObject.type === 'udp_video.imagery' && options.strategy !== 'latest';
    },
    request: async (domainObject, options) => {
      console.log("get image historic provider", options)
...
function getLadProvider(openmct) {
  return {
    ...
    request: async (domainObject, options) => {
      // console.log("get image lad provider", options)
